...
from client.remote import (RemoteEnv,
                      PutFileToRemoteTask,
                      CheckFileExistsRemoteTask,
                      DelFileFromRemoteTask,
                      GetFileFromRemoteTask,
                      RunInRemoteTask)
from client.machine import VMachine, VMStatus
from .settings import TEST_VM_HOST
...
class RemoteActionTestCase(TestCase):
    ...
    def test_get_file(self):
        self.vm.create()
        self.vm.putfile(fixture_file_path, self._fixture_remote_path)
        self.vm.getfile(self._fixture_remote_path, self._fixture_remote_path)

        self.assertTrue(os.path.exists(self._fixture_remote_path))
        os.remove(self._fixture_remote_path)
